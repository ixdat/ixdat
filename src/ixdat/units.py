...
ureg.autoconvert_offset_to_baseunit = True  # This is to help with logarithmic plotting.
#  I dont know if it is better to simply set the dimensions to "dimensionless"
#  prior to plotting with units
...
def _register_format_function(plotter_type):
    """Make a default formatter to dynamic update matplotlib labels
    when units are updated on axes"""
    try:
...
def _area_calibration(A_el):
    ...
    c.add_transformation(
        "[substance]/[time]/[length]**2", "[substance]/[time]", lambda ureg, x: x * A_el
    )
    c.add_transformation(
        "[substance]/[time]", "[substance]/[time]/[length]**2", lambda ureg, x: x / A_el
    )
    ureg.add_context(c)
...
STANDARD_LABELS_BY_PLOTTER = {
    "DEFAULT": {
        ...
        # formatter never reaches this due to pints handeling of dimensionless units
    },
    "LOG": {
